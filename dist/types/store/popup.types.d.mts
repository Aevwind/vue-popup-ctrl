...
export type PopupInstance<Name extends string> = ComponentName<Name> extends keyof PopupComps ? PopupComps[ComponentName<Name>] extends abstract new (...args: any[]) => infer Instance ? Instance : ComponentPublicInstance : ComponentPublicInstance;
type ComponentProps<Name extends string> = PopupInstance<Name> extends {
    $props: infer Props;
} ? Props : Record<string, any>;
export type DefineProps<Name extends string> = ComponentName<Name> extends PopupCompsKey ? Prettify<Omit<ComponentProps<Name>, keyof PublicProps | CapitalizedKeys<ComponentProps<Name>, 'on'>>> : Record<string, any>;
export type DefineEmits<Name extends string> = ComponentName<Name> extends PopupCompsKey ? {
    [K in Exclude<CapitalizedKeys<ComponentProps<Name>, 'on'>, keyof PublicProps> as K extends `on${infer Event}` ? Uncapitalize<Event> : never]: NonNullable<ComponentProps<Name>[K]>;
